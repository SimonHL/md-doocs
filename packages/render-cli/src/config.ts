 ...
 */
function findConfigFile(explicitPath: string | undefined, mdFilePath: string): string | null {
    // 1. 命令行显式指定
    if (explicitPath) {
    ...
    for (const key of Object.keys(source) as (keyof T)[]) {
        ...
        if (sourceValue !== undefined) {
            if (
                ...
                result[key] = deepMerge(result[key] as Record<string, any>, sourceValue as Record<string, any>) as T[keyof T]
            } else {
 ...
 * @returns 合并后的配置对象
 */
export async function loadConfig(explicitPath: string | undefined, mdFilePath: string): Promise<RenderConfig> {
 ...
 * 获取配置文件路径（用于测试）
 */
export function getConfigPath(explicitPath: string | undefined, mdFilePath: string): string | null {
