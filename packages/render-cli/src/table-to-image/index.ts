...
export async function processTablesInHtml(
  ...
  if (mdFilePath) {
    const ensured = ensureImagesDir(mdFilePath)
    outputDir = ensured.outputDir
    relativeDir = ensured.relativeDir
    if (ensured.created) {
      ...
      if (canUpload) {
        try {
          ...
          if (
            ...
            && uploadError.message.includes('40164')
          ) {
            console.error(`\n❌ ${uploadError.message}`)
            console.error(
              '🛑 因 IP 白名单限制，已停止后续上传。将仅生成本地预览文件。\n',
            )
