 ...
 */
export async function generateOnlineMarkdown(
  mdFilePath: string,
  config: RenderConfig,
): Promise<string> {
  ...
  let match: RegExpExecArray | null = mdImageRegex.exec(content)
  while (match !== null) {
  ...
  for (const item of replacements) {
    ...
    // 跳过远程图片和 Base64
    if (isRemoteAssetSource(src)) {
      continue
    }

    const localPath = resolveMdAssetPath(mdFilePath, src)
 ...
 */
export function writeOnlineMarkdown(content: string, mdFilePath: string): void {
