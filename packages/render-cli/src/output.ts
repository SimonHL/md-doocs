 ...
 */
export function inlineTableStyles(
  html: string,
  ...
  const styles: TableStyles = {
    table:
      'width:100%; border-collapse:collapse; margin:1.15em 0; font-size:0.9375em; border:2px solid #2d2d2d;',
...
export function inlineCodeBlockStyles(html: string): string {
  ...
  // 外层容器样式 (确保与图片容器对齐)
  const wrapperStyles
    = 'display: block; overflow-x: auto !important; overflow-y: hidden; background: #282c34; border-radius: 8px; margin: 10px 0 !important; padding: 0; -webkit-overflow-scrolling: touch;'

  // 代码块样式 (WeChat 兼容：-webkit-box 负责块级布局，nowrap 应用于 code)
  // 注意：pre 不能应用 white-space: nowrap，否则会导致 mac-sign 无法正确占据顶部空间
  const preStyles
    = 'margin: 0 !important; padding: 0 !important; border: none !important; border-radius: 0 !important; background: transparent !important; word-wrap: normal !important; word-break: normal !important; overflow: visible !important;'

  const codeStyles
    = 'display: -webkit-box !important; padding: 10px 10px 15px !important; white-space: nowrap !important; word-wrap: normal !important; overflow-x: visible !important; font-family: Consolas, Monaco, \'Andale Mono\', \'Ubuntu Mono\', monospace !important; min-width: 100%; box-sizing: border-box !important;'
...
export function inlineImageStyles(html: string): string {
  ...
  const imageWrapperStyles
    = 'margin: 10px 0 !important; padding: 0 !important; overflow: hidden; display: block; border: none !important;'
  ...
  $('img').each((_, el) => {
    ...
    if ($parentFigure.length > 0) {
      ...
      $parentFigure.attr(
        'style',
        `${currentFigureStyle} ${imageWrapperStyles}`,
      )
    }
    else {
      ...
      if ($parentSection.length === 0) {
        ...
        $parentSection.attr(
          'style',
          `${currentSectionStyle} ${imageWrapperStyles}`,
        )
      }
    }
  })
  ...
  $('figcaption').each((_, el) => {
    $(el).attr(
      'style',
      'text-align: center; color: #888; font-size: 0.8em; margin-top: 5px;',
    )
  })
 ...
 * 将 HTML 写入文件
 */
export function writeOutput(html: string, outputPath: string): void {
