...
import { ensureImagesDir } from './utils/path-helpers.js'
...
export async function prepareMermaidData(
  ...
  if (mdFilePath) {
    outputDir = ensureImagesDir(mdFilePath).outputDir
  }
  else {
  ...
  mermaidElements.each((i, el) => {
    if (i < images.length) {
      $(el).replaceWith(
        `<mermaid-img-holder index="${i}"></mermaid-img-holder>`,
      )
    }
  })
 ...
 */
export function injectMermaidImages(
  html: string,
  images: MermaidImage[],
  ...
  let result = html
  const holderRegex
    = /<mermaid-img-holder\s+index="(\d+)"\s*(?:\/>|>\s*<\/mermaid-img-holder>)/gi
...
async function renderMermaidToImages(
  ...
  console.log(
    `🎨 使用 Playwright 渲染 ${codes.length} 个 Mermaid 图表为图片...`,
  )
...
<body>
    ${codes
      .map((code, i) => `<div id="mermaid-${i}" class="mermaid">${code}</div>`)
      .join('\n')}
    <script>
        mermaid.initialize({ 
...
export async function processPlantUMLDiagrams(html: string): Promise<string> {
  ...
  console.log(
    `🌱 发现 ${plantumlElements.length} 个 PlantUML 图表，开始处理...`,
  )
    ...
    if ($img.length > 0) {
      ...
      if (svgUrl && svgUrl.includes('plantuml.com')) {
        try {
          ...
          console.error(
            `  ⚠️ PlantUML 图表 ${i + 1} SVG 获取失败，保留图片链接`,
          )
        }
      }
    }
