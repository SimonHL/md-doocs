...
program
  ...
  .action(async (file: string, options: CLIOptions) => {
    try {
      ...
      } = await import('./diagram-renderer.js')

      const mermaidData = await prepareMermaidData(
        processedHtml,
        result.rawMarkdown,
        config,
        file,
      ...
      if (options.tableImage !== false) {
        ...
        const processResult = await processTablesInHtml(
          processedHtml,
          mergedCSS,
          config,
          file,
      ...
      if (wechatHtml) {
        ...
        writeOutput(fullHtml, outputPath)
